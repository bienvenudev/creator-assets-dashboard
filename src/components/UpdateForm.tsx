...
import { validateAssetUpdateForm, type ValidationErrors } from '../utils/validation';
...
export function UpdateForm({ onSubmit, onCancel, initialAsset }: UpdateFormProps) {
  const [formData, setFormData] = useState<FormData>({
    name: initialAsset.name,
    category: initialAsset.category,
    description: initialAsset.description,
    tags: initialAsset.tags.join(', '),
    file: null,
  });
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    // Validate form (file is optional for updates)
    const validationErrors = validateAssetUpdateForm(formData);
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
      <div className="bg-white rounded-lg max-w-2xl w-full max-h-[90vh] overflow-y-auto">
        ...
        <div className="flex items-center justify-between p-6 border-b">
          <h2 className="text-2xl font-bold text-gray-900">Update Asset</h2>
          <button
            onClick={onCancel}
